        ...
        struct WebViewCleanModal: View {
            ...
            var body: some View {
                ...
                struct WebViewCleanModal: View {
                    ...
                    @State private var lastJSResult: String? = nil
                    @State private var showNoBackAlert = false
                    @State private var showJSResultAlert = false
                    ...
                    var body: some View {
                        ...
                        .alert(isPresented: $showNoBackAlert) {
                            Alert(title: Text("Nessuna navigazione indietro disponibile"),
                                  message: Text("Non è stato possibile eseguire il comportamento di indietro nella pagina."),
                                  dismissButton: .default(Text("OK")))
                        }
                        .alert(isPresented: $showJSResultAlert) {
                            Alert(title: Text("Test Back result"),
                                  message: Text(lastJSResult ?? "(nessun risultato)"),
                                  dismissButton: .default(Text("OK")))
                        }
                    }
                    ...
                    private func triggerPageBack(testOnly: Bool = false) {
                        ...
                        let js = "(function(){try{var sels=[\(selectorsArrayString)];for(var i=0;i<sels.length;i++){var el=document.querySelector(sels[i]);if(el){el.click();return 'clicked:'+sels[i];}}if(window.customBack){try{window.customBack();return 'customBack';}catch(e){}}if(window.history && window.history.length>1){window.history.back();return 'history.back';}return 'no-op';}catch(e){return 'error:'+e.toString();}})()"
...
struct WebViewClean: UIViewRepresentable {
    ...
    // Placeholder file removed: canonical implementation lives in Views/WebViewClean.swift
    // This file intentionally left minimal to avoid duplicate-sources parsing issues.

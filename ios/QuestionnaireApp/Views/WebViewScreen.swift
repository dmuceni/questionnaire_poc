...
struct WebViewScreen: View {
    ...
    var body: some View {
        NavigationView {
            ZStack { WebView(url: url, store: store) }
    ...
    private func runTestBackJS() { triggerPageBack(testOnly: true) }
...
struct WebView: UIViewRepresentable {
    ...
    init(url: URL, store: WebViewStore) { self.url = url; self.store = store }
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        DispatchQueue.main.async { self.store.webView = wk }
        return wk
    }
